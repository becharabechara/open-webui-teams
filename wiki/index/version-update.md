...
1. **Determine the New Version Tag**:
   - The current latest version of OpenWebUI is `v0.6.34`. For this update, use `v0.6.34` as the new version tag.

2. **Build the Docker Image**:
   - In your project directory, build the Docker image with the new version tag and also tag it as `latest`:
     ```
     docker build -t BachByte-open-webui:v0.6.34 -t BachByte-open-webui:latest .
     ```
...
3. **Test with Local Docker Compose**:
   - Update the `docker-compose.yaml` file to use the newly built image:
     ...
     services:
       open-webui:
         image: BachByte-open-webui:v0.6.34
...
1. **Stage and Commit Changes**:
   ...
   - Commit the changes with a descriptive message:
     ```
     git commit -m "Update OpenWebUI to v0.6.34 with upstream changes"
     ```
...
- **Note on Docker Push**: You do not need to manually push the Docker image to Azure Container Registry (ACR). Azure DevOps pipelines are configured to handle the build and push of the `BachByte-open-webui:v0.6.34` and `BachByte-open-webui:latest` images to ACR as part of the deployment process.
